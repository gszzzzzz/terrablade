...
func TestDiagnosticMessagesFromParse(t *testing.T) {
	for _, test := range []struct {
		...
		{"a=\"x\ny\"", syntax.NewlineInQuotedTemplate, "A quoted template cannot contain a literal newline; use a heredoc instead."},
		...
		{"a={for x in xs:x}", syntax.ExpectedForArrow, "Expected '=>' between the key and value of an object for expression."},
		{"a=[for x in xs:x=>x]", syntax.UnexpectedForKey, "A tuple for expression cannot produce a key."},
		{"a=[for x in xs:x...]", syntax.UnexpectedForGrouping, "Grouping with '...' is allowed only in an object for expression."},
		...
		{`a="%{unknown}"`, syntax.UnknownTemplateDirective, "Unknown template directive; expected 'if', 'else', 'endif', 'for', or 'endfor'."},
		{`a="%{else}"`, syntax.UnexpectedTemplateDirective, "The template directive has no matching open scope or repeats an 'else'."},
