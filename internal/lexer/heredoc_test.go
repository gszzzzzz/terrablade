...
func TestHeredocs(t *testing.T) {
	for _, test := range []struct {
		...
		{"template sequences", "<<E\n%{~if x}${y~}%{endif}\nE\n", []tokenText{{HeredocOpen, "<<"}, {HeredocMarker, "E"}, {Newline, "\n"}, {DirectiveOpen, "%{"}, {StripMarker, "~"}, {Identifier, "if"}, {Whitespace, " "}, {Identifier, "x"}, {TemplateSequenceEnd, "}"}, {InterpolationOpen, "${"}, {Identifier, "y"}, {StripMarker, "~"}, {TemplateSequenceEnd, "}"}, {DirectiveOpen, "%{"}, {Identifier, "endif"}, {TemplateSequenceEnd, "}"}, {TemplateText, "\n"}, {HeredocEndMarker, "E"}, {Newline, "\n"}}},
		{"marker after interpolation is text", "<<E\n${x}E\nE\n", []tokenText{{HeredocOpen, "<<"}, {HeredocMarker, "E"}, {Newline, "\n"}, {InterpolationOpen, "${"}, {Identifier, "x"}, {TemplateSequenceEnd, "}"}, {TemplateText, "E\n"}, {HeredocEndMarker, "E"}, {Newline, "\n"}}},
		{"nested heredoc", "<<E\n${<<E\ninner\nE\n}outer\nE\nx=1", []tokenText{{HeredocOpen, "<<"}, {HeredocMarker, "E"}, {Newline, "\n"}, {InterpolationOpen, "${"}, {HeredocOpen, "<<"}, {HeredocMarker, "E"}, {Newline, "\n"}, {TemplateText, "inner\n"}, {HeredocEndMarker, "E"}, {Newline, "\n"}, {TemplateSequenceEnd, "}"}, {TemplateText, "outer\n"}, {HeredocEndMarker, "E"}, {Newline, "\n"}, {Identifier, "x"}, {Equal, "="}, {Number, "1"}}},
		{"heredoc inside quote", "\"${<<E\nx\nE\n}\"", []tokenText{{QuoteOpen, `"`}, {InterpolationOpen, "${"}, {HeredocOpen, "<<"}, {HeredocMarker, "E"}, {Newline, "\n"}, {TemplateText, "x\n"}, {HeredocEndMarker, "E"}, {Newline, "\n"}, {TemplateSequenceEnd, "}"}, {QuoteClose, `"`}}},
		{"quote inside heredoc", "<<E\n${\"text\"}\nE\n", []tokenText{{HeredocOpen, "<<"}, {HeredocMarker, "E"}, {Newline, "\n"}, {InterpolationOpen, "${"}, {QuoteOpen, `"`}, {TemplateText, "text"}, {QuoteClose, `"`}, {TemplateSequenceEnd, "}"}, {TemplateText, "\n"}, {HeredocEndMarker, "E"}, {Newline, "\n"}}},
		{"not a heredoc", "<<-EOT", []tokenText{{Less, "<"}, {Less, "<"}, {Minus, "-"}, {Identifier, "EOT"}}},
		{"spaced opener", "<< EOT\n", []tokenText{{Less, "<"}, {Less, "<"}, {Whitespace, " "}, {Identifier, "EOT"}, {Newline, "\n"}}},
		{"trailing opener space", "<<EOT \n", []tokenText{{Less, "<"}, {Less, "<"}, {Identifier, "EOT"}, {Whitespace, " "}, {Newline, "\n"}}},
		...
		for _, closing := range []string{"  EOT", "EOT \t", "\tEOT\t ", "\u00A0EOT\u2003"} {
			for _, newline := range []string{"\n", "\r\n"} {
				...
				t.Run(source, func(t *testing.T) {
					assertTokens(t, source, []tokenText{{HeredocOpen, opener}, {HeredocMarker, "EOT"}, {Newline, newline}, {TemplateText, "text" + newline}, {HeredocEndMarker, closing}, {Newline, newline}, {Identifier, "x"}, {Equal, "="}, {Number, "1"}})
				})
			}
		}
	}
