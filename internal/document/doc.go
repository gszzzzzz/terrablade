...
// Empty constructors and a zero Doc render as empty. Invalid text, negative
// options, and integer layout overflow panic because these are
// construction/configuration errors, not parse errors.
//
// # Width measurement
//
...
// nodes are measured together. Tabs use fixed tab stops. Other control
// characters follow the width library's zero-width rules; this is not a
// terminal emulator and ANSI escape sequences are not interpreted. Actual
// fonts/terminals can display some characters differently. PrintWidth is a
// preference: Text is never split, and indentation or unbreakable text can
// exceed it.
//
// # Alignment
//
...
// # Ownership
//
// Constructors hide storage and copy mutable inputs. Strings and child
// documents may share immutable storage, keeping their backing memory alive.
// No builder, arena ownership, or mutable traversal interface is exposed.
// Rendering owns its output and work stacks, so shared Docs are safe for
// concurrent reads.
//
// # Performance
//
// Construction and rendering use no recursive traversal and impose no
// arbitrary nesting limit. Resources scale with expanded document size, not
// just shared node count. Group probes borrow the pending continuation without
// copying it. Independent groups separated by lines take linear work;
// pathological nested groups can revisit overlapping contents and take
// quadratic time. A long grapheme split across many Text nodes can also take
// quadratic time. Alignment takes linear work in the output size per populated
// column; there are at most 256 columns. Each cell's prefix is measured once
// for its column.
package document
