// Format's resource contract: the fuzz target, concurrency, and scaling tests.
	...
	for range 8 {
		workers.Go(func() {
			for range 20 {
				...
				snapshot := shared.Diagnostics()
				if !reflect.DeepEqual(snapshot, diagnostics) || shared.Error() == "" {
